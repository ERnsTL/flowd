...
impl Component for FlowdVisComponent {
    fn new(mut inports: ProcessInports, _: ProcessOutports, signals_in: ProcessSignalSource, signals_out: ProcessSignalSink, graph_inout: GraphInportOutportHandle) -> Self where Self: Sized {
        FlowdVisComponent {
        ...
        loop {
            ...
            while !inn.is_empty() {
                if let Ok(_ip) = inn.pop() {
                    ...
                    flowd_component_api::send_network_previewurl_comfortable(&self.graph_inout, "https://placehold.co/150x150".to_string());
                } else {
                    break;
                }
            }
        ...
        ComponentComponentPayload {
            ...
            in_ports: vec![
                ComponentPort {
                    ...
                    required: true,
                    is_arrayport: false,
                    description: String::from("packets to trigger preview URL sending to graph -> clients"),
                    values_allowed: vec![],
