        ...
        match self.state {
            TelegramBotState::WaitingForConfig => {
                ...
                if let Ok(conf_vec) = self.conf.pop() {
                    ...
                    let token_str = conf_vec.as_text().expect("failed to parse token as text");
            ...
            TelegramBotState::Active => {
                    ...
                    if let Ok(result) = self.result_receiver.try_recv() {
                        match result {
                            ...
                            TelegramBotResult::MessageReceived(msg_bytes, chat_id) => {
                                ...
                                let message_msg = FbpMessage::from_bytes(msg_bytes);
                                if let Err(_) = self.out.push(message_msg) {
                ...
                if context.remaining_budget > 0 {
                    if let Ok(msg_msg) = self.inn.pop() {
                        debug!("Received message to send to Telegram");
                        let text = msg_msg.as_text().expect("failed to convert message to text");

                        // Send message command to async thread
                        if let Err(_) = self.cmd_sender.send(TelegramBotCommand::SendMessage(text.to_string()))
                        {
