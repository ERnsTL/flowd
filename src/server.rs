...
pub fn run() -> Result<()> {
    ...
    let (loaded_graphs, active_graph) =
        crate::load_or_create_graph_set().expect("failed to load or create graphs");
...
fn must_not_block<Role: tungstenite::handshake::HandshakeRole>(
    err: tungstenite::HandshakeError<Role>,
) -> tungstenite::Error {
    match err {
...
impl FlowdServer {
    ...
    pub fn start(&mut self) -> Result<()> {
        ...
        flag::register(SIGINT, self.sigint_received.clone())
            .expect("failed to register SIGINT handler");
        flag::register(SIGTERM, self.sigterm_received.clone())
                ...
                Ok(stream) => {
                    ...
                    thread::Builder::new()
                        ...
                        .spawn(move || {
                            log::info!(
                                "got a client from {}",
                                stream.peer_addr().expect("get peer address failed")
                            );
        ...
        if self.sigterm_received.load(Ordering::Relaxed) {
            ...
            if let Err(e) = self
                ...
                .read()
                .unwrap()
                .persist(&self.graph.read().unwrap())
            {
    ...
    fn handle_client(
        ...
        fn has_readonly_secret_for_graph(
            runtime: &Arc<RwLock<Runtime>>,
            secret: Option<&String>,
            graph: &str,
        ) -> bool {
        ...
        */
        let peer_addr = stream.peer_addr().expect("could not get peer socketaddr");
        {
            ...
            graph_inout
                .lock()
                .expect(r#"could not acquire lock for saving TcpStream for graph outport process"#)
                .websockets
                .insert(
                    peer_addr,
                    Arc::new(Mutex::new(tungstenite::WebSocket::from_raw_socket(
                        cloned_stream,
                        tungstenite::protocol::Role::Server,
                        None,
                    ))),
        ...
        loop {
            ...
            match websocket.read()? {
                msg @ Message::Text(_) | msg @ Message::Binary(_) => {
                    ...
                    let mut json_envelope: serde_json::Value =
                    ...
                    match fbpmsg {
                        ...
                        FBPMessage::Runtime(RuntimeMessage::Getruntime(payload)) => {
                            ...
                            if validate_secret_readonly(
                                &runtime,
                                payload.secret.as_ref(),
                                &runtime.read().expect("lock poisoned").graph,
                            )
                            .is_err()
                            {
                                websocket
                                    .send(Message::text(
                                        serde_json::to_string(&RuntimeErrorResponse::new(
                            ...
                            if has_readonly_secret_for_graph(
                                &runtime,
                                payload.secret.as_ref(),
                                &runtime_payload.graph,
                            ) && !runtime_payload
                                .capabilities
                                .contains(&Capability::GraphReadonly)
                            {
                            ...
                            websocket
                                .send(Message::text(
                                    serde_json::to_string(&RuntimePortsMessage::new(
                                        &*runtime.read().expect("lock poisoned"),
                                        &graph.read().expect("lock poisoned"),
                                    ))
                        ...
                        FBPMessage::Component(ComponentMessage::List(_payload)) => {
                            ...
                            .is_err()
                            {
                                websocket
                                    .send(Message::text(
                                        serde_json::to_string(&GraphErrorResponse::new(
                                            "invalid secret token".to_string(),
                                        ))
                            ...
                            let mut count: u32 = 0;
                            for component in
                                components.read().expect("lock poisoned").available.iter()
                            {
                                log::info!("response: sending component:component message");
                                websocket
                                    .send(Message::text(
                                        serde_json::to_string(&ComponentComponentMessage::new(
                                            &component,
                                        ))
                            ...
                            websocket
                                .send(Message::text(
                                    serde_json::to_string(&ComponentComponentsreadyMessage::new(
                                        count,
                                    ))
                                    .expect(
                        ...
                        FBPMessage::Network(NetworkMessage::Getstatus(_payload)) => {
                            ...
                            websocket
                                .send(Message::text(
                                    serde_json::to_string(&NetworkStatusMessage::new(
                        ...
                        FBPMessage::Network(NetworkMessage::Persist(_payload)) => {
                            ...
                            match runtime
                                .read()
                                .expect("lock poisoned")
                                .persist(&graph.read().expect("lock poisoned"))
                            {
                                // NOTE: persist() now stores all known graphs from the runtime graph manager
                                Ok(_) => {
                                    log::info!("response: sending network:persist message");
                                    websocket
                                        .send(Message::text(
                                            serde_json::to_string(
                                                &NetworkPersistResponse::default(),
                                            )
                                        ...
                                        .send(Message::text(
                                            serde_json::to_string(&NetworkErrorResponse::new(
                                                ...
                                                runtime
                                                    ...
                                                    .expect("lock poisoned")
                                                    .graph
                                                    .clone(), // there is no field "graph" in the payload that could re-used here
                                            ))
                            ...
                            if graph.read().expect("lock poisoned").properties.name == payload.name
                            {
                                ...
                                log::debug!(
                                    ...
                                    graph
                                        .write()
                                        .expect("lock poisoned")
                                        .get_source(payload.name.clone())
                                        .expect("could not get graph source")
                                        .code
                                );
                                match graph
                                    .write()
                                    .expect("lock poisoned")
                                    .get_source(payload.name)
                                {
                                    Ok(source_info) => {
                                        log::info!(
                                            "response: sending component:source message for graph"
                                        );
                                        websocket
                                            .send(Message::text(
                                                serde_json::to_string(
                                                    &ComponentSourceMessage::new(source_info),
                                                )
                                                .expect(
                                                    "failed to serialize component:source message",
                                                ),
                                            ))
                                            .expect("failed to write message into websocket");
                                    }
                                ...
                                log::info!(
                                    "got a request for component source of {}",
                                    &payload.name
                                );
                                match components
                                    .read()
                        ...
                        FBPMessage::Graph(GraphMessage::Clear(payload)) => {
                            ...
                            if validate_secret(&runtime, payload.secret.as_ref(), &payload.name)
                                .is_err()
                            {
                                websocket
                                    .send(Message::text(
                            ...
                            match graph
                                .write()
                                .expect("lock poisoned")
                                .clear(&payload, &*runtime_write)
                            {
                                Ok(_) => {
                                    runtime_write.graph = payload.name.clone();
                                    runtime_write
                                        .graphs
                                        .add_graph(payload.name.clone(), graph.clone());
                                    ...
                                    websocket
                                        .send(Message::text(
                                            serde_json::to_string(&GraphClearResponse::new(
                                                &payload,
                                            ))
                        ...
                        FBPMessage::Graph(GraphMessage::Addnode(payload)) => {
                            ...
                            if let Some(graph_name) = payload.graph.clone() {
                                if validate_secret(&runtime, payload.secret.as_ref(), &graph_name)
                                    .is_err()
                                {
                                    websocket
                                        .send(Message::text(
                                ...
                                let target_graph =
                                    match get_graph_by_name(&runtime, &graph, &graph_name) {
                                        Ok(graph_arc) => graph_arc,
                                        Err(_err) => {
                                            websocket
                                                .send(Message::text(
                                                    serde_json::to_string(
                                                        &GraphErrorResponse::new(String::from(
                                                            "Requested graph not found",
                                                        )),
                                                    )
                                                    .expect(
                                                        "failed to serialize graph:error response",
                                                    ),
                                                ))
                                                .expect("failed to write message into websocket");
                                            continue;
                                ...
                                match add_node_result {
                                    Ok(response) => {
                                        ...
                                        websocket
                                            .send(Message::text(
                                                serde_json::to_string(&GraphAddnodeResponse::new(
                                                    response,
                                                ))
                                                .expect(
                                                    "failed to serialize graph:addnode response",
                                                ),
                                            ))
                                            .expect("failed to write message into websocket");
                                    }
                                ...
                                websocket
                                    .send(Message::text(
                                        serde_json::to_string(&GraphErrorResponse::new(
                                            String::from("No graph specified"),
                                        ))
                                        .expect("failed to serialize graph:error response"),
                                    ))
                                    .expect("failed to write message into websocket");
                        ...
                        FBPMessage::Graph(GraphMessage::Removenode(payload)) => {
                            ...
                            if validate_secret(&runtime, payload.secret.as_ref(), &payload.graph)
                                .is_err()
                            {
                                websocket
                                    .send(Message::text(
                            ...
                            let target_graph =
                                match get_graph_by_name(&runtime, &graph, &payload.graph) {
                                    Ok(graph_arc) => graph_arc,
                                    Err(err) => {
                                        websocket
                                            .send(Message::text(
                            ...
                            match remove_node_result {
                                Ok(removed_edges) => {
                                    for removed_edge in removed_edges {
                                        websocket
                                            .send(Message::text(
                                                ...
                                                .expect(
                                                    "failed to serialize graph:removeedge response",
                                                ),
                                            ))
                                            .expect("failed to write message into websocket");
                                    }
                                    ...
                                    websocket
                                        .send(Message::text(
                                            serde_json::to_string(
                                                &GraphRemovenodeResponse::from_request(payload),
                                            )
                                            .expect(
                                                "failed to serialize graph:removenode response",
                                            ),
                                        ))
                                        .expect("failed to write message into websocket");
                                }
                            ...
                            match rename_node_result {
                                Ok(_) => {
                                    ...
                                    websocket
                                        .send(Message::text(
                                            serde_json::to_string(
                                                &GraphRenamenodeResponse::from_request(payload),
                                            )
                                            .expect(
                                                "failed to serialize graph:renamenode response",
                                            ),
                                        ))
                                        .expect("failed to write message into websocket");
                                }
                                ...
                                Ok(updated_metadata) => {
                                    ...
                                    websocket
                                        .send(Message::text(
                                            ...
                                            .expect(
                                                "failed to serialize graph:changenode response",
                                            ),
                                        ))
                                        .expect("failed to write message into websocket");
                                }
                            ...
                            match add_edge_result {
                                Ok(_) => {
                                    ...
                                    websocket
                                        .send(Message::text(
                                            serde_json::to_string(
                                                &GraphAddedgeResponse::from_request(payload),
                                            )
                            ...
                            match remove_edge_result {
                                ...
                                Ok(_) => {
                                    ...
                                    websocket
                                        .send(Message::text(
                                            serde_json::to_string(
                                                &GraphRemoveedgeResponse::from_request(payload),
                                            )
                                            .expect(
                                                "failed to serialize graph:removeedge response",
                                            ),
                            ...
                            match change_edge_result {
                                ...
                                Ok(_) => {
                                    ...
                                    websocket
                                        .send(Message::text(
                                            serde_json::to_string(
                                                &GraphChangeedgeResponse::from_request(payload),
                                            ) //TODO optimize clone
                                            .expect(
                                                "failed to serialize graph:changeedge response",
                                            ),
                                        ))
                                        .expect("failed to write message into websocket");
                                }
                            ...
                            match add_initial_result {
                                Ok(_) => {
                                    ...
                                    websocket
                                        .send(Message::text(
                                            serde_json::to_string(
                                                &GraphAddinitialResponse::from_request(&payload),
                                            )
                                            .expect(
                                                "failed to serialize graph:addinitial response",
                                            ),
                                        ))
                                        .expect("failed to write message into websocket");
                                }
                            ...
                            match remove_initial_result {
                                Ok(removed_src) => {
                                    ...
                                    websocket
                                        .send(Message::text(
                                            serde_json::to_string(
                                                &GraphRemoveinitialResponse::from_removed(
                                                    payload.graph,
                                                    removed_src,
                                                    payload.tgt,
                                                ),
                                            ...
                                            .expect(
                                                "failed to serialize graph:removeinitial response",
                                            ),
                                        ))
                                        .expect("failed to write message into websocket");
                                }
                                    ...
                                    websocket
                                        .send(Message::text(
                                            serde_json::to_string(&response).expect(
                                                "failed to serialize graph:addinport response",
                                            ),
                                        ))
                                        .expect("failed to write message into websocket");
                                }
                            ...
                            match remove_inport_result {
                                Ok(_) => {
                                    ...
                                    websocket
                                        .send(Message::text(
                                            serde_json::to_string(
                                                &GraphRemoveinportResponse::default(),
                                            )
                                            .expect(
                                                "failed to serialize graph:removeinport response",
                                            ),
                                        ))
                                        .expect("failed to write message into websocket");
                                }
                            ...
                            match rename_inport_result {
                                Ok(_) => {
                                    websocket
                                        .send(Message::text(
                                            serde_json::to_string(
                                                &GraphRenameinportResponse::default(),
                                            )
                                            .expect(
                                                "failed to serialize graph:renameinport response",
                                            ),
                                        ))
                                        .expect("failed to write message into websocket");
                                }
                            ...
                            match add_outport_result {
                                Ok(_) => {
                                    ...
                                    websocket
                                        .send(Message::text(
                                            serde_json::to_string(&response).expect(
                                                "failed to serialize graph:addoutport response",
                                            ),
                                        ))
                                        .expect("failed to write message into websocket");
                                }
                            ...
                            match remove_outport_result {
                                Ok(_) => {
                                    ...
                                    websocket
                                        .send(Message::text(
                                            serde_json::to_string(
                                                &GraphRemoveoutportResponse::from_request(payload),
                                            )
                                            .expect(
                                                "failed to serialize graph:removeoutport response",
                                            ),
                                        ))
                                        .expect("failed to write message into websocket");
                                }
                            ...
                            match rename_outport_result {
                                Ok(_) => {
                                    websocket
                                        .send(Message::text(
                                            serde_json::to_string(
                                                &GraphRenameoutportResponse::default(),
                                            )
                                            .expect(
                                                "failed to serialize graph:renameoutport response",
                                            ),
                                        ))
                                        .expect("failed to write message into websocket");
                                }
                                ...
                                Ok(_) => {
                                    ...
                                    websocket
                                        .send(
                                            Message::text(
                                                serde_json::to_string(
                                                    &GraphAddgroupResponse::default(),
                                                )
                                                .expect(
                                                    "failed to serialize graph:addgroup response",
                                                ),
                                            ),
                                        )
                                        .expect("failed to write message into websocket");
                            ...
                            match remove_group_result {
                                Ok(_) => {
                                    ...
                                    websocket
                                        .send(Message::text(
                                            serde_json::to_string(
                                                &GraphRemovegroupResponse::default(),
                                            )
                                            .expect(
                                                "failed to serialize graph:removegroup response",
                                            ),
                                        ))
                                        .expect("failed to write message into websocket");
                                }
                            ...
                            match rename_group_result {
                                Ok(_) => {
                                    ...
                                    websocket
                                        .send(Message::text(
                                            serde_json::to_string(
                                                &GraphRenamegroupResponse::default(),
                                            )
                                            .expect(
                                                "failed to serialize graph:renamegroup response",
                                            ),
                                        ))
                                        .expect("failed to write message into websocket");
                                }
                            ...
                            match change_group_result {
                                Ok(_) => {
                                    ...
                                    websocket
                                        .send(Message::text(
                                            serde_json::to_string(
                                                &GraphChangegroupResponse::default(),
                                            )
                                            .expect(
                                                "failed to serialize graph:changegroup response",
                                            ),
                                        ))
                                        .expect("failed to write message into websocket");
                                }
                        ...
                        FBPMessage::Trace(TraceMessage::Start(payload)) => {
                                ...
                                .is_err()
                            {
                                websocket
                                    .send(Message::text(
                                        serde_json::to_string(&TraceErrorResponse::new(
                                            "invalid secret token".to_string(),
                                        ))
                            ...
                            match runtime
                                .write()
                                .expect("lock poisoned")
                                .start_trace(payload.graph.as_str(), payload.buffer_size)
                            {
                                Ok(_) => {
                                    log::info!("response: sending trace:start response");
                                    websocket
                                        .send(Message::text(
                                            serde_json::to_string(&TraceStartResponse::new(
                                                payload.graph,
                                            ))
                        ...
                        FBPMessage::Trace(TraceMessage::Stop(payload)) => {
                            ...
                            match runtime
                                .write()
                                .expect("lock poisoned")
                                .stop_trace(payload.graph.as_str())
                            {
                                Ok(_) => {
                                    log::info!("response: sending trace:stop response");
                                    websocket
                                        .send(Message::text(
                                            serde_json::to_string(&TraceStopResponse::new(
                                                payload.graph,
                                            ))
                        ...
                        FBPMessage::Trace(TraceMessage::Clear(payload)) => {
                            ...
                            match runtime
                                .write()
                                .expect("lock poisoned")
                                .clear_trace(payload.graph.as_str())
                            {
                                Ok(_) => {
                                    log::info!("response: sending trace:clear response");
                                    websocket
                                        .send(Message::text(
                                            serde_json::to_string(&TraceClearResponse::new(
                                                payload.graph,
                                            ))
                        ...
                        FBPMessage::Trace(TraceMessage::Dump(payload)) => {
                            ...
                            match runtime
                                .write()
                                .expect("lock poisoned")
                                .dump_trace(&payload.graph)
                            {
                                Ok(dump) => {
                                    log::info!("response: sending trace:dump response");
                                    websocket
                                        .send(Message::text(
                                            serde_json::to_string(&TraceDumpResponse::new(
                                                payload.graph,
                                                dump,
                                            ))
                        ...
                        FBPMessage::Runtime(RuntimeMessage::Packet(payload)) => {
                            ...
                            match Runtime::packet(&payload, graph_inout.clone(), runtime.clone()) {
                                Ok(_) => {
                                    ...
                                    websocket
                                        .send(Message::text(
                                            serde_json::to_string(&RuntimePacketsentMessage::new(
                                                RuntimePacketsentPayload::from(payload),
                                            ))
                                            .expect(
                                                "failed to serialize runtime:packetsent response",
                                            ),
                                        ))
                                        .expect("failed to write message into websocket");
                                }
                        ...
                        FBPMessage::Runtime(RuntimeMessage::Packetsent(_payload)) => {
                            ...
                            websocket
                                .send(Message::text(
                                    serde_json::to_string(&RuntimeErrorResponse::new(
                                        String::from("runtime:packetsent from client is an error"),
                                    ))
                                    .expect("failed to serialize runtime:error response"),
                                ))
                                .expect("failed to write message into websocket");
                        ...
                        FBPMessage::Network(NetworkMessage::Edges(payload)) => {
                            ...
                            match runtime
                                .write()
                                .expect("lock poisoned")
                                .set_debug_edges(&payload.graph, &payload.edges)
                            {
                                Ok(_) => {
                                    ...
                                    websocket
                                        .send(Message::text(
                                            serde_json::to_string(
                                                &NetworkEdgesResponse::from_request(payload),
                                            )
                                    ...
                                    Err(_) => {
                                        websocket
                                            .send(Message::text(
                                                ...
                                                .expect(
                                                    "failed to serialize network:error response",
                                                ),
                                            ))
                                            .expect("failed to write message into websocket");
                                        continue;
                        ...
                        FBPMessage::Network(NetworkMessage::Stop(payload)) => {
                            ...
                            if runtime.read().expect("lock poisoned").status.graph != payload.graph
                            {
                                websocket
                                    .send(Message::text(
                                        serde_json::to_string(&NetworkErrorResponse::new(
                                            String::from(
                                                "requested graph is not the running network",
                                            ),
                                            String::from(""),
                                            payload.graph,
                                        ))
                            ...
                            if !runtime.read().expect("lock poisoned").status.running {
                                ...
                                log::info!(
                                    "response: sending network:stopped response (already stopped)"
                                );
                                websocket
                                    .send(Message::text(
                        ...
                        FBPMessage::Network(NetworkMessage::Debug(payload)) => {
                            ...
                            match runtime
                                .write()
                                .expect("lock poisoned")
                                .debug_mode(payload.graph.as_str(), payload.enable)
                            {
                                Ok(_) => {
