...
pub fn main() {
    ...
    let componentlib: Arc<RwLock<ComponentLibrary>> = build_component_library();
    //TODO actually load components
    info!("component library initialized");

    // graph inport and outport holder
    let graph_inout: Arc<Mutex<GraphInportOutportHolder>> = Arc::new(Mutex::new(GraphInportOutportHolder { inports: None, outports: None, websockets: HashMap::new(), packet_tap: None}));
    // graph itself
